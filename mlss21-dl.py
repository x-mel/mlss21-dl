...
def input_index():
  while True:
    try:
       list_idx = [int(item) for item in input("\n Choose the lectures you want to download separated by a space (0 means all of them):\n").split()]
    except ValueError:
...
for day in lectures.keys():
    for session in lectures[day]:
        print(session['index'], ':', session['speaker'], '-', session['title'] )

list_idx = input_index()

for day in lectures.keys():
    for session in lectures[day]:
        if (session['index'] in list_idx or 0 in list_idx): 
